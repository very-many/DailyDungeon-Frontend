...
export class DungeonGame {
    ...
    private emptyGrid(): boolean[][] {
        ...
        return this.puzzle.solution.map((row: number[]): boolean[] =>
            row.map(() => false),
        );
    }
    ...
    private renderGrid(): void {
        ...
        grid.addEventListener("contextmenu", (event: Event): void =>
    ...
    private buildCell(
        ...
        if (row === 0 && col === 0) {
            ...
            this.colCounts[x] = this.buildCountLabel(
            ...
            const y = row - 1;
            cell.className = HEADER_CELL;
            this.rowCounts[y] = this.buildCountLabel(
                puzzle.wall_counts.rows[y],
            );
    ...
    private buildPlayCell(cell: HTMLDivElement, x: number, y: number): void {
        ...
        cell.addEventListener(
            "touchstart",
            (event: TouchEvent): void => this.onCellTouchStart(event, x, y),
            {
                passive: false,
            },
        );
        cell.addEventListener("mousedown", (event: MouseEvent): void =>
            this.onCellMouseDown(event, x, y),
        );
    ...
    private buildSprite(occupant: CellOccupant): HTMLSpanElement {
        ...
        if (occupant === "treasure") {
            ...
            sprite.className =
                "monster scale-100 sm:scale-200 bg-position-[50%]";
    ...
    private refreshWallStyles(): void {
        ...
        for (let y = 0; y < puzzle.height; y++) {
            for (let x = 0; x < puzzle.width; x++) {
                ...
                if (!cell) continue;
                cell.className =
                    rowsOver[y] || colsOver[x] ? WALL_CELL_OVER : WALL_CELL;
            }
        }
    }
    ...
    private wallsPlacedInRow(y: number): number {
        return this.walls[y].filter((isWall: boolean): boolean => isWall)
            .length;
    }
    ...
    private onTouchMove(event: TouchEvent): void {
        ...
        const cell =
            target instanceof HTMLElement
                ? target.closest<HTMLElement>("[data-x][data-y]")
                : null;
        if (!cell) return;
    ...
    private isSolved(): boolean {
        ...
        return this.walls.every((row: boolean[], y: number): boolean =>
            row.every(
                (isWall: boolean, x: number): boolean =>
                    (isWall ? 1 : 0) === solution[y][x],
            ),
        );
    }
}
